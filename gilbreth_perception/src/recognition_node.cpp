...
void loadModel(std::vector<std::vector<double> > &pick_pose, std::vector<std::string> &model_name,
		...
		std::vector<pcl::PointCloud<pcl::SHOT352>::Ptr> &model_descriptor_list, std::vector<pcl::PointCloud<pcl::ReferenceFrame>::Ptr> &model_rf_list) {
	// Load model settings
	...
	// if use correspondence grouping
		...
		for (int i = 0; i < model_list.size(); i++) {
			...
			descr_est.setInputCloud (model_list[i]);
			...
			rf_est.setInputCloud (model_list[i]);
...
void cloudCallBack(const sensor_msgs::PointCloud2ConstPtr &cloud_msg, const tf::TransformListener &listener,
	...
	if(icp){
		...
		for (int j = 0; j < model_list.size(); j++){
			...
			clusterer.setInputCloud (model_list[j]);
			...
			std::cerr<<"Correspondence number: "<<clustered_corrs[0].size()<<std::endl;
			}
			else{std::cerr<<"Correspondence number: 0"<<std::endl;}
			if(clustered_corrs.size()>0 && clustered_corrs[0].size()>min_index){
...
int main(int argc, char **argv) {
	...
	ros::NodeHandle nh;
	loadParameter();
	loadModel(pick_pose, model_name, model_list, model_features_list,model_descriptor_list,model_rf_list);
	tf::TransformListener listener;
	// Create a ROS subscriber for the input point cloud
	ros::Subscriber sub_1 = nh.subscribe<sensor_msgs::PointCloud2>("segmentation_result", 100, boost::bind(cloudCallBack, _1, boost::ref(listener), pick_pose, model_name, model_list, model_features_list,model_descriptor_list,model_rf_list));
	// ROS publisher
